...
static uint8_t cur_tlp = 0;
static uint8_t tlp_size = 10;
static uint8_t top_level_pages[] = {
    GUI_PAGE_MAIN,
    GUI_PAGE_STICKS,
    GUI_PAGE_SETTINGS,
    GUI_PAGE_SETUP_MAIN,
    GUI_PAGE_SETUP_CLONETX,
    GUI_PAGE_SETUP_BIND,
    GUI_PAGE_SETUP_BOOTLOADER,
    GUI_PAGE_CONFIG_MAIN,
    GUI_PAGE_CONFIG_STICK_CAL,
    GUI_PAGE_CONFIG_MODEL_SETTINGS};
// internal functions
...
static void gui_handle_buttons(void)
{
    ...
    gui_page = top_level_pages[cur_tlp];
}
...
void gui_loop(void)
{
    ...
    while (gui_shutdown_pressed < GUI_SHUTDOWN_PRESS_COUNT)
    {
        ...
        // debug("CH3: ");
        // debug_put_int8(adc_get_channel_rescaled(CHANNEL_ID_CH3));
        // debug_put_newline();
        // debug_put_newline();
        // debug_flush();
        // render ui
...
#define GUI_RSSI_BAR_W 25
    ...
    if (bar_w > 1)
        bar_w--;
    if (bar_w > 0)
        screen_fill_rect(1 + bar_w, 2, 25 - bar_w, 3, 1);
    bar_w = min(rssi, 100) / 4;
    if (bar_w > 1)
        bar_w--;
    if (bar_w > 0)
        screen_fill_rect(x + bar_w, 2, 25 - bar_w, 3, 1);
}
...
static void gui_setup_clonetx_render(void)
{
    ...
    if (gui_config_counter >= 1)
        screen_puts_xy(3, 9 + 2 * h, 1, "preparing autotune");
    if (gui_config_counter >= 2)
        screen_puts_xy(3, 9 + 3 * h, 1, "autotune running (takes long)");
    ...
    if (gui_config_counter >= 4)
        screen_puts_xy(3, 9 + 5 * h, 1, "fetching hoptable (takes long)");
    ...
    if (gui_config_counter >= 7)
        screen_puts_xy(3, 9 + 7 * h, 1, "done. please switch off now");
...
static void gui_config_stick_calibration_render(void)
{
    ...
    uint32_t y = 8;
    //                       |                             |
    screen_puts_xy(3, y, 1, "Please move all sticks to the");
    y += h;
    screen_puts_xy(3, y, 1, "extreme positions.");
    y += h;
    screen_puts_xy(3, y, 1, "When done, move all sticks to");
    y += h;
    screen_puts_xy(3, y, 1, "the center and press save.");
    y += h;
