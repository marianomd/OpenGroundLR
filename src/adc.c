...
uint16_t adc_get_channel(uint32_t id) {
    ...
    if (config_hw_revision == CONFIG_HW_REVISION_I6S) {
        ...
        switch (id) {
            ...
            case (CHANNEL_ID_CH4)      : return adc_data[6];
            case (CHANNEL_ID_CH5)      : return adc_data[7];
...
char *adc_get_channel_name(uint8_t i, bool short_descr) {
    switch (i) {
        ...
        case (CHANNEL_ID_CH4)       : return ((short_descr) ? "4" : "CH4");
        case (CHANNEL_ID_CH5)       : return ((short_descr) ? "5" : "CH5");       
    }
}
